...
if (empty($error_code)) {
    ...
    if (empty($recipient_data)) {
		...
		if (!empty($data['followers'])) {
			$response_data['followers'] = Wo_GetFollowers($recipient_data['user_id']);
		}
		if (!empty($data['following'])) {
			$response_data['following'] = Wo_GetFollowing($recipient_data['user_id']);
		}
		if (!empty($data['liked_pages'])) {
			$response_data['liked_pages'] = Wo_GetLikes($recipient_data['user_id']);
		}
		if (!empty($data['joined_groups'])) {
			$response_data['joined_groups'] = Wo_GetUsersGroups($recipient_data['user_id']);
		}
    }
}