...
function Wo_GetUserFromSessionID($session_id, $platform = 'web') {
    ...
    $query      = mysqli_query($sqlConnect, "SELECT `user_id` FROM " . T_APP_SESSIONS . " WHERE `session_id` = '{$session_id}'");
...
function Wo_RegisterNotification($data = array()) {
    ...
    if (!isset($data['undo']) or $data['undo'] != true) {
            ...
            if (!empty($admins)) {
                foreach ($admins as $admin) {
                    if ($admin['user_id'] != $wo['user']['user_id']) {
                        ...
                        $admin_ids[] = "('$admin_id', '$my_id', '$admin_post_id','" . $data['type'] . "','" . $data['type2'] . "','" . $data['text'] . "','{$url}'," . time() . ")";
                    }
                }
            }
        }
        if (!empty($admin_ids)) {
            ...
            $query_admins = "INSERT INTO " . T_NOTIFICATION . " (`recipient_id`, `notifier_id`, `post_id`, `type`, `type2`, `text`, `url`, `time`) VALUES ";
...
function Wo_ShareFile($data = array(), $type = 0) {
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        if ($second_file == 'jpg' || $second_file == 'jpeg' || $second_file == 'png' || $second_file == 'gif') {
            ...
            if ($type == 1) {
                ...
                if (Wo_Resize_Crop_Image(400, 400, $filename, $last_file, 60)) {
                    if (($wo['config']['amazone_s3'] == 1 || $wo['config']['ftp_upload'] == 1) && !empty($last_file)) {
                        $upload_s3 = Wo_UploadToS3($last_file);
                    }
                }
            } else {
        ...
        if (($wo['config']['amazone_s3'] == 1 || $wo['config']['ftp_upload'] == 1) && !empty($filename)) {
            $upload_s3 = Wo_UploadToS3($filename);
        }
        $last_data             = array();
...
function Wo_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0)) {
    ...
    if (isset($Wo_publisher['user_id']) && is_numeric($Wo_publisher['user_id'])) {
    ...
    } else if (isset($Wo_page_publisher['page_id']) && is_numeric($Wo_page_publisher['page_id'])) {
    ...
    } else if (isset($Wo_group_publisher['id']) && is_numeric($Wo_group_publisher['id'])) {
    ...
    } else if (isset($Wo_event_publisher['id']) && is_numeric($Wo_event_publisher['id'])) {
