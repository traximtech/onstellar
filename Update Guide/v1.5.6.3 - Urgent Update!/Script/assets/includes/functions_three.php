...
function Wo_GetFriendsStatus($data_array = array('limit' => 8, 'user_id' => 0)) {
    ...
    $query     = "SELECT * FROM " . T_USER_STORY . " WHERE (user_id IN (SELECT following_id FROM " . T_FOLLOWERS . " WHERE follower_id = '$user_id') OR user_id = $user_id) AND user_id IN (SELECT user_id FROM " . T_USERS . " WHERE active = '1') GROUP BY user_id ORDER BY id DESC LIMIT " . $data_array['limit'];
    $query_run = mysqli_query($sqlConnect, $query);
