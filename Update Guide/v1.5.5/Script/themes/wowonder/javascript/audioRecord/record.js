...
function Wo_InsertComment(dataForm,post_id){
    ...
    $.ajax({
      ...
      if(data.status == 200) {
        Wo_CleanRecordNodes();
        post_wrapper.find('.comment-container:last-child').after(data.html);
        post_wrapper.find('.comments-list-lightbox').find('.comment-container:first-child').before(data.html);
